...
@WebServlet("/ItemCategoryServlet2")
@MultipartConfig(location="/tmp", maxFileSize=192048576) //画像アップロードに必要
public class ItemCategoryServlet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("修正2"+request.getParameter("修正"));
		System.out.println("delete2"+request.getParameter("delete"));
		System.out.println("delail2"+request.getParameter("delail"));
		if(Jbk.getKey() == null) {
			if(request.getParameter("修正") != null) {
				...
				Jbk.setKey(request.getParameter("detail"));
				Jbk.setNumKey(Integer.valueOf(Jbk.getKey()));
				Jbk.setKey(null);
				request.setAttribute("Jbk",Jbk);
				 System.out.println("numkey" + Jbk.getNumKey());
			...
			List<Item> ctgList = f.execute(Jbk);
			session.setAttribute("ctgList", ctgList);
			System.out.println("ctgList_Jbk.getKey() != null" + ctgList);
			//if(numKey > 0) {
				//RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/itemChanged.jsp");
				//dispatcher.forward(request, response);
			//}
			RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/itemCategory2.jsp");
			dispatcher.forward(request, response);
		}
